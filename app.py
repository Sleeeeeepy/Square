...
def show_my_ranking(user_id):
    ...
    if prev_score_user > max_score_user:
        ...
        db.users.update_one(
            {"userid": user_id}, {"$set": {"max_score": prev_score_user}}
        )
        # max score date를 갱신한다
        current_time = datetime.now()
        formatted_time = current_time.strftime("%Y-%m-%d %H:%M:%S")
        db.users.update_one(
            {"userid": user_id}, {"$set": {"max_score_date": formatted_time}}
        )
...
def signup_process():
    if request.method == "POST":
        ...
        if not re.match(pattern, userid):
            errors["useridError"] = "아이디는 영문 및 숫자로 구성되어야 합니다"
...
def home2():
    ...
    return render_template(
        "login.html", max_score=request.current_user.get("max_score", 0)
    )
...
def game(user_id):
    return render_template(
        "game.html", current_user_name=request.current_user.get("name")
    )
...
def process_achievement(user_id):
    ...
    if 77.77 in scores:
        if check_and_achieve("1", user_id):
            title = "777"
            body = "77.77점 달성하기"
            return make_achievement_response(title, body), 200
...
def check_and_achieve(achievement_id, user_id):
    if (
        db.achievement.find_one({"achievementid": achievement_id, "userid": user_id})
        == None
    ):
        db.achievement.insert_one(
            {"achievementid": achievement_id, "userid": user_id, "date": datetime.now()}
        )
        return True
    return False
